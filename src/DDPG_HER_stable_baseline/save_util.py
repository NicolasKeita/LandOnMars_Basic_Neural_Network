...
def save_to_zip_file(
    ...
    if policy_net_weights is None:
        raise ValueError("Policy network weights not found in the provided params dictionary.")
    ...
    with zipfile.ZipFile(file, mode="w") as archive:
        ...
        # Save the policy network weights as a JSON file
        ...
        # policy_net_weights = dict(policy_net_weights)
        with archive.open(json_path, mode="w", force_zip64=True) as json_file:

            # json.dump(policy_net_weights, json_file)
            json_file.write(json.dumps(policy_net_weights).encode('utf-8'))
